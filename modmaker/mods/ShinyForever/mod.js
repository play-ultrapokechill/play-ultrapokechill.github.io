const MOD_ID = "shinyForever";
const PATCH_KEY = "__shinyForeverPatch";
...
function isUnlockedPokemon(pokemon) {
  return Math.max(0, Math.floor(Number(pokemon?.caught) || 0)) > 0 || pokemon?.newPokemon === true;
}
...
function getFormFamilyKey(id, canonicalIds, lowerToId) {
  const stripped = stripFormAffixes(id);
  const strippedLower = stripped.toLowerCase();

  for (const candidate of canonicalIds) {
    if (candidate.length < 4 || candidate === id) continue;
    const lowerCandidate = candidate.toLowerCase();
    if (!strippedLower.startsWith(lowerCandidate)) continue;
    const rest = strippedLower.slice(lowerCandidate.length);
...
function stripFormAffixes(id) {
  let value = String(id || "");
  const prefixes = ["alolan", "galarian", "hisuian", "paldean", "mega", "primal", "spiky"];
  const lower = () => value.charAt(0).toLowerCase() + value.slice(1);

  for (const prefix of prefixes) {
    if (!lower().startsWith(prefix) || value.length <= prefix.length) continue;
    value = value.slice(prefix.length);
    value = value.charAt(0).toLowerCase() + value.slice(1);
    break;
  }

  if (/^[a-z0-9]+[XY]$/.test(value)) value = value.slice(0, -1);
  return value;
}
...
function updateAreaEndSprites() {
  ...
  for (const card of list.querySelectorAll("[data-pkmn-editor]")) {
    ...
    if (!card.querySelector("[data-shiny-forever-tag]")) {
